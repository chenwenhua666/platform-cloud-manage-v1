...
export default {
  ...
  data() {
    return {
      ...
      // 是否上传失败
      isUploadError: false,
      action: `${process.env.VUE_APP_BASE_API}/foundation/file/attachment/upload`,
      headers: {
  ...
  methods: {
      ...
      if (file.response) {
        ...
        if (vm.acceptSize) {
          ...
          if (isLtAcceptSize) {
            setTimeout(() => {
              vm.$message.error(
                '只能上传' + vm.renderSize(vm.acceptSize) + '的文件!已为您过滤文件：' + file.name
              )
            }, 10)
    ...
    async deleteAttachment() {
      const vm = this
      const res = await this.$delete('/foundation/file/attachment', {
    ...
    async getAttachment() {
      const vm = this
      const res = await this.$get('/foundation/file/attachment', {
