<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-dropdown
        v-has-any-permission="['file:attachment:upload', 'file:attachment:delete', 'file:attachment:download']"
        class="filter-item"
        trigger="click"
      >
        <el-button>
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item
            v-has-permission="['file:attachment:upload']"
            @click.native="upload"
          >
            {{ $t("table.upload") }}
          </el-dropdown-item>
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          ...
          />
          <el-link
            v-has-no-permission="['file:attachment:download', 'file:attachment:delete']"
            class="no-perm"
          >
            {{ $t("tips.noPermission") }}
          </el-link>
...
export default {
  ...
  methods: {
    ...
    download(ids) {
      this.$get('foundation/file/attachment/download', { ids: ids }).then(response => {
        ...
        if (type.includes('application/json')) {
          ...
          var fileName = '下载文件.zip' // eslint-disable-line no-unused-vars
          if (disposition) {
    ...
    delete(ids) {
      this.$delete('foundation/file/attachment', { ids: ids }).then(response => {
        const res = response.data
        if (res.isSuccess) {
          this.$message({
    ...
    fetch(params = {}) {
      ...
      this.$get('foundation/file/attachment/page', {
        ...params
      }).then(response => {
        const res = response.data
        this.loading = false
